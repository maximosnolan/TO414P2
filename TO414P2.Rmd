...
# ANN Prediction
```{r}
ANN_model1 <- neuralnet(formula = is_canceled ~ ., data = train)
plot(ANN_model1)
ANN_pred1 <- predict(ANN_model1, test)
threshold <- 0.5
predicted_yyes_1 <- ifelse(ANN_pred1 < threshold, 0, 1)
CrossTable(x = test$is_canceled, y = predicted_yyes_1, prop.chisq=FALSE)
confusionMatrix(as.factor(test$is_canceled), as.factor(predicted_yyes_1), positive = "1")
```

**Summary**
We can see from the ANN model output, there are 2633 false positive and 9389 false negative. The overall accuracy is 79.86%.
...
```{r}
sms_train = hoteldata_norm[train_id1, -match("is_canceled",names(hoteldata_norm))]
sms_test = hoteldata_norm[-train_id1, -match("is_canceled",names(hoteldata_norm))]
sms_train_labels = as.factor(hoteldata_norm[train_id1,  "is_canceled"])
sms_test_labels = as.factor(hoteldata_norm[-train_id1, "is_canceled"])
