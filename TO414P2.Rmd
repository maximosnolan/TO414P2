...
pred_lr <- predict(canceled_lr, hotel_test, type="response")
...
```{r}
set.seed(12345)
# Peform the tuned KNN to get the best K value
...
combined_predictions <- data.frame(pred_lr, knn_predict, ANN_pred2, pred_dt_boost, sms_test_pred2, predictions_svm, hotel_test$is_canceled)

#Use 70% as training, 30% as testing.
set.seed(12345)
train_ID = sample(0.7*nrow(hoteldata_norm))
stacked_train = hoteldata_norm[train_ID, ]
stacked_test = hoteldata_norm[-train_ID, ]
model <- C5.0(as.factor(group1_test.yyes) ~ ., data = train, trails = 10)
plot(model)
summary(model)
